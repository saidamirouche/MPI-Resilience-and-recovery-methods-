...
#+begin_example
mpirun -am ft-enable-mpi -np 6 LI_jacobi >> li.txt 
mpirun -am ft-enable-mpi -np 6 reset_jacobi >> reset.txt 
mpirun -am ft-enable-mpi -np 6 jacobi_check_restart >> er.txt
mpirun -am ft-enable-mpi -np 6 jacobi_non_fault >> nf.txt
#+end_example

this plot was done by [[https://doc.ubuntu-fr.org/octave][OCTAVE]], using a [[https://github.com/samirouche/ft_uflm/blob/master/graphe.m][script]] to plot from different outputs of the source codes and it shows the different behavor of the recovery methods (LI, Reset, ER) compared with the Nan-fault methode (NF) when killing 4 processes
...
- Killing the same processes for the second time :

  + despite that the other processes has knowledge for the victim rank each time ( broadcasting it each time ), in line 433 of [[https://github.com/samirouche/ft_uflm/blob/master/ft_src/contre_examples/double_kill.c][this code]] i've tried to kill many times the same process but i get this error

#+begin_example
mpirun -am ft-enable-mpi -np 6  double_kill
#+end_example
...
- Killing the process 0 blocks the execution
  ...
  + each time i kill the process 0 it blocks the execution, the illustration in [[https://github.com/samirouche/ft_uflm/blob/master/ft_src/contre_examples/master_proc.c][this code]] in the line 433 , and results it stays blocked in the execution.

 #+begin_example
mpirun -am ft-enable-mpi -np 6  master_proc
#+end_example


- measure the time with MPI_Wtime() after killing a process

  + in order to calculate the necessary time for the convergency , i summed each time the time elapsed after each iteration, but once the first suicide happens it shows me errors
as [[https://github.com/samirouche/ft_uflm/blob/master/ft_src/contre_examples/time_ulfm.c][showen in this code]] in the lines 314 and 467. and the errors in execution 

 #+begin_example
mpirun -am ft-enable-mpi -np 6  time_ulfm
#+end_example
